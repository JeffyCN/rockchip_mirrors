...
if [ ! -e "/data/cfg/rockchip" ]; then
	...
	mkdir /data/cfg/rockchip
fi

if [ ! -e "/data/cfg/rockchip/auto_reboot.sh" ]; then
	cp /rockchip/auto_reboot/auto_reboot.sh /data/cfg/rockchip
        echo $total > /data/cfg/rockchip/reboot_total_cnt
    sync
fi
...
then
    ...
    echo "off" > $CNT
    echo "do cleaning ..."
    rm -rf /data/cfg/rockchip/auto_reboot.sh
    rm -rf /data/cfg/rockchip/reboot_total_cnt
    rm -f $CNT
    sync
    exit 0
fi
